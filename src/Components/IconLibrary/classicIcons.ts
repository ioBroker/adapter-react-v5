...
export interface ClassicIconTemplate {
    _id: string;
    name: ioBroker.StringOrTranslated;
    /** File name of the icon in src/assets/rooms or src/assets/devices */
    icon: string;
}

export const CLASSIC_DEVICES: ClassicIconTemplate[] = [
    {
        _id: 'hood',
        name: {
            en: 'Hood',
...
export const CLASSIC_ROOMS: ClassicIconTemplate[] = [
    {
        _id: 'storeroom',
        name: {
            en: 'Storeroom',
...
/**
 * Get the classic room or device templates with their icons as SVG data URL.
 * They define an ID, a translated name and an icon, e.g., to create a room or function enum.
 *
 * @param type rooms or devices
 */
export function getClassicIconTemplates(type: 'rooms' | 'devices'): ClassicIconTemplate[] {
    const icons: Record<string, string> = type === 'rooms' ? roomsIcons : devicesIcons;
    return (type === 'rooms' ? CLASSIC_ROOMS : CLASSIC_DEVICES).map(item => ({
        ...item,
        icon: `data:image/svg+xml;base64,${icons[item.icon.replace(/\.svg$/, '')]}`,
    }));
}
